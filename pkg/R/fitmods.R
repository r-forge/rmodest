fitmods <- function(d,cx=rep(1,length(d)),models=c('g','gm','l','lm'),ext=F){
  ...
  t <- cbind(t,LL=NA,a=NA,b=NA,c=NA,s=NA); 
  fc <- function(x){
    pf <- parent.frame(3); mo <- as.character(t$complex[x[1]]);
    if(all(t$complex[x]=='e')) {pf$t$LL[x] <- nd*(log(lambda)-1); pf$t$a[x] <- lambda; return();}
    if(length(x)>1){
    ...
    o <- opsurv(d,model=mo,par=modpars(unlist(p),modeli=mi,modelo=mo,trim=T),lb=c(1e-14,0,0,0),cx=cx);
