...
`opsurv` <-
function(model,x,y=NULL,par=rep(1e-6,4),cons=1,usegr=T,usehs=F,debug=F,
		lb=c(1e-20,1e-11,1e-11,1e-8),ub=c(.5,1,.5,3),called=NULL,cx=NULL,cy=NULL,
		mvers='',method='nm',debugcomment=NULL,...){
	callargs<-as.list(environment(),all.names=T);
	if(!exists("rm.debug")){rm.debug<<-list(0);} 
	call<-sys.call(); tstart<-proc.time()[3];
	switch(model,
