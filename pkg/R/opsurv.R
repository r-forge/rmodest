...
function(x,y=NULL,model='g',par=c(2.6e-6,.004,1e-7,0.1),cons=1,usegr=T,usehs=F,debug=F,
	 lb=c(1e-14,1e-4,0,0),ub=c(.5,.5,.5,2),called=NULL,cx=NULL,cy=NULL,
	 mvers='',method="Nelder-Mead",minout= -Inf,tlog=F,...){
...
g, gm, l, and lm.")
	...
	if(mean(cons)==1 & !is.null(y)){
		out<-list();
		options('warn'= -1);
		out0<-opsurv(x,model=model,par=par,cons=cons,usegr=usegr,usehs=usehs,debug=debug,lb=lb,ub=ub,called=called,cx=cx,cy=cy,mvers=mvers,method=method,tlog=tlog);
			     if(length(par)==2*np){par1<-par[(np+1):(2*np)];
			     } else {if(length(par)==8){par1<-par[5:8];
			     } else par1<-par;}
		out1<-opsurv(y,model=model,par=par1,cons=cons,usegr=usegr,usehs=usehs,debug=debug,lb=lb,ub=ub,called=called,cx=cx,cy=cy,mvers=mvers,method=method,tlog=tlog);
		options('warn'=0);
...
and use the defaults.");}
		...
		fn<-function(par){
			objf(par,lb=lb,ub=ub,cons=rep(cons,le=4),x=x,y=y,np=np,keep=keep[1:np],minout=minout,ex=ex,nx=nx,ny=ny,cx=cx,cy=cy,tlog=tlog);
		}
		...
		while(max(change)>0){
			...
			if(class(out)[1]!="try-error"){
				...
				if(max(p-log(lb))<1e-10){browser();cat('X\n');print(p);print(exp(lb));print(lb);break();}
				}else{
				p<-p/2; lbp<-p>lb;
				...
				lk<-objf(p,lb=lb,ub=ub,cons=rep(cons,le=4),
					x=max(x),y=maxy,np=np,keep=keep[1:np],ex=ex,nx=1,ny=1,cx=1,cy=1,tlog=tlog);
